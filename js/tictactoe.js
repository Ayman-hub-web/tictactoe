...
function fillShape(id) {
    if (id == 0 || !fields[id] && !gameOver) {
        if (currentShape == 'cross') {
            ...
            document.getElementById('player-2').classList.add('opci');

        } else {
            currentShape = 'cross';
            document.getElementById('player-2').classList.remove('opci');
            document.getElementById('player-1').classList.add('opci');
